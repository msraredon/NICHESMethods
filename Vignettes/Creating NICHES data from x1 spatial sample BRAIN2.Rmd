---
title: "Running NICHES on x1 spatial sample BRAIN 2"
author: "MSBR"
date: "`r Sys.Date()`"
...
```{r}
SpatialFeaturePlot(brain2, features = "nCount_Spatial")
```
...
```{r}
SpatialFeaturePlot(brain2, features = "nFeature_Spatial")
```
...
```{r}
SpatialFeaturePlot(brain2, features = "Fgf1",slot = 'counts')
```
...
genes.to.impute <- rownames(brain2)[rowSums(brain2@assays$Spatial@counts>0)>num.cells.per.feature]

# Run imputation only on selected genes
brain2 <- SeuratWrappers::RunALRA(brain2)
```
...
```{r}
NICHES_alra <- RunNICHES(object = brain2,
                           LR.database = "fantom5",
                           species = "mouse",
                           ...
                           meta.data.to.map = names(brain2@meta.data), # this maps over all meta.data
...
```{r}
NICHES_norm <- RunNICHES(object = brain2,
                           LR.database = "fantom5",
                           species = "mouse",
...
brain2[["NeighborhoodToCell_ALRA"]] <- CreateAssayObject(data = alra.data)
brain2[["NeighborhoodToCell_Norm"]] <- CreateAssayObject(data = alra.data)

# Add additional metadata
brain2 <- AddMetaData(brain2,metadata = niches.data$alra@meta.data$nCount_NeighborhoodToCell,col.name = 'nCount_NeighborhoodToCell_ALRA')
brain2 <- AddMetaData(brain2,metadata = niches.data$alra@meta.data$nFeature_NeighborhoodToCell,col.name = 'nFeature_NeighborhoodToCell_ALRA')
brain2 <- AddMetaData(brain2,metadata = niches.data$norm@meta.data$nCount_NeighborhoodToCell,col.name = 'nCount_NeighborhoodToCell_Norm')
brain2 <- AddMetaData(brain2,metadata = niches.data$norm@meta.data$nFeature_NeighborhoodToCell,col.name = 'nFeature_NeighborhoodToCell_Norm')
...
```{r}
p1 <- VlnPlot(brain2,'nCount_Spatial')+scale_y_continuous(trans = 'log10',limits = c(1,40000))
p2 <- VlnPlot(brain2,'nFeature_Spatial')+scale_y_continuous(trans = 'log10',limits = c(1,40000))
p3 <- VlnPlot(brain2,'nFeature_NeighborhoodToCell_Norm')+ylim(0,1000)
p4 <- VlnPlot(brain2,'nFeature_NeighborhoodToCell_ALRA')+ylim(0,1000)
...
```{r}
p1 <- SpatialFeaturePlot(brain2,'nCount_Spatial')+scale_color_continuous(trans = 'log10',limits = c(1,40000))
p2 <- SpatialFeaturePlot(brain2,'nFeature_Spatial')+scale_color_continuous(trans = 'log10',limits = c(1,40000))
p3 <- SpatialFeaturePlot(brain2,'nFeature_NeighborhoodToCell_Norm')+scale_color_continuous(limits = c(0,1000))
p4 <- SpatialFeaturePlot(brain2,'nFeature_NeighborhoodToCell_ALRA')+scale_color_continuous(limits = c(0,1000))
cowplot::plot_grid(p1,p2,p3,p4,nrow=1)
```
...
## Save
```{r eval=FALSE}
save(brain2,file = '/Users/msbr/Library/CloudStorage/GoogleDrive-michasam.raredon@yale.edu/My Drive/Large_Files_Public/brain2/brain2.NICHES.Robj')
```
